...
A common routine during data analysis is to select only certain *observations*. 
The `data_filter()` is designed to achieve this. `data_filter()` is a wrapper 
...
Additionally, one often need to select only a few attributes/columns for further analysis.
To this end, `data_select()` can be useful; it is the equivalent of `dplyr::select()`. 
