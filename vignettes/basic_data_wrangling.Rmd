...
`datawizard` is a package whose aim is to make basic data wrangling easier than 
with base R. Although it is inspired by the tidyverse combo `dplyr` + `tidyr`, 
...
ecosystem. One drawback of this approach is that some important features of the 
`tidyverse` packages are not supported and we will have to rely on base R, or on 
the package [`poorman`](https://github.com/nathaneastwood/poorman/). It is also
important to note that `datawizard` was designed so that it avoids namespace 
conflicts with `tidyverse` packages.

In this vignette, we will see how to go through basic data wrangling steps with 
`datawizard` (code with a <span style = "background-color: #e6e6ff;">light purple
background</span>). We will also compare the `dplyr` + `tidyr` syntax (code with
a <span style = "background-color: #d9f2e5;">light green background</span>), 
so that you can easily see how to translate the `tidyverse` syntax if you decide
to make the switch. This vignette is largely inspired from `dplyr`'s [Getting 
...
`data_rename()` is the equivalent of `dplyr::rename()`. The syntax between the 
two is different though. While `dplyr::rename()` takes each new-old pair of column
...
`reshape_longer()` and `reshape_wider()`, which are largely inspired from 
`tidyr::pivot_longer()` and `tidyr::pivot_wider()`. We will use the `relig_income`
dataset, as in the [`tidyr` vignette](https://tidyr.tidyverse.org/articles/pivot.html).
...
fish_encounters %>% 
  reshape_wider(
    colnames_from = "station", 
    values_from = "seen"
  ) %>% 
  convert_na_to(replace_num = 0)
```
:::
