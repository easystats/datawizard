...
`data_filter()` is a wrapper 
around `subset()`. Therefore, if you want to have several filtering conditions, 
you need to use `&`. Separating the conditions with a comma (as in `dplyr::filter()`) 
will **not** work; it will only apply the first condition.
