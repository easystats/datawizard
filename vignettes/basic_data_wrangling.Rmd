...
`tidyverse` packages are supported and we will have to rely on base R, or on 
...
`datawizard`. We will also compare it to the `dplyr` + `tidyr` syntax, so that 
you can easily see how to translate the `tidyverse` syntax if you decide to make
the switch. This vignette is largely inspired from `dplyr`'s [Getting started vignette](https://dplyr.tidyverse.org/articles/dplyr.html).
