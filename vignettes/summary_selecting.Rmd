...
You can also use formulas to select variables:

```{r}
get_columns(iris, ~ Sepal.Length + Petal.Width)
```

This was mostly useful to use selection more easily in custom functions before
...
We can pass a function to `select`. This function will be applied to all columns
and has to return `TRUE` or `FALSE`. For example, if we want to keep only numeric
columns, we can use `is.numeric`.

```{r}
get_columns(iris, is.numeric)
```

Note that we can also define our own functions and pass them in `select`, 
...
on variable positions in the dataset as they may change later. In this case, we
...
possibilities as `select`. Note that we can use both `select` and `exclude` at 
the same time.
...
of patterns and we want to relocate columns based on these pattern, one by one:
...
In the loop above, all columns starting with `"Sep"` were moved at the end of the data frame, and the same thing was made with all columns starting with `"Pet"`.
