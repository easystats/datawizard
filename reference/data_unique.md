...
From all rows with at least one duplicated ID, keep only one. Methods
for selecting the duplicated row are either the first duplicate, the
last duplicate, or the "best" duplicate (default), based on the
duplicate with the smallest number of `NA`. In case of ties, it picks
the first duplicate, as it is the one most likely to be valid and
...
- keep:
  ...
  The method to be used for duplicate selection, either "best" (the
